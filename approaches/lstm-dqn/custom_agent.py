...
Transition = namedtuple('Transition', ('observation_id_list',
                                       'word_indices',
                                       'reward',
                                       'mask',
                                       'done',
...
class CustomAgent:
    def __init__(self):
        ...
        self.replay_memory = PrioritizedReplayMemory(self.config['general']['replay_memory_capacity'],
                                                     priority_fraction=self.config['general'][
    ...
    def choose_maxQ_command(self, word_ranks, word_masks_np):
        ...
        batch_size = word_ranks[0].size(0)
        word_ranks_np = [to_np(item) for item in word_ranks]  # list of batch x n_vocab
        word_ranks_np = [r - np.min(r) for r in
                         word_ranks_np]  # minus the min value, so that all values are non-negative
        word_ranks_np = [r * m for r, m in zip(word_ranks_np, word_masks_np)]  # list of batch x n_vocab
        ...
        if self.current_step > 0:
            for b in range(len(obs)):
                ...
                self.replay_memory.push(is_prior, self.cache_description_id_list[b],
                                        [item[b] for item in self.cache_chosen_indices], rewards[b], mask[b], dones[b],
                                        description_id_list[b], [item[b] for item in self.word_masks_np])
    ...
    def update(self):
        ...
        next_observation_id_list = pad_sequences(batch.next_observation_id_list,
        ...
        word_qvalues = [w_rank.gather(1, idx).squeeze(-1) for w_rank, idx in
                        zip(word_ranks, chosen_indices)]  # list of batch
